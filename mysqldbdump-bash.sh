...
OUTPUT_FILE_DIR=${OUTPUT_DIR}/DBBACKUPS/proddb_dump_${DATE}.sql.gz
...
then
	...
	curl -X POST -L -H "Authorization: Bearer "$ACCESS_TOKEN""  -F 'metadata={name : "'"$OUTPUT_NAME_GZ"'"};type=application/json;charset=UTF-8' -F "file=@$OUTPUT_FILE_DIR;type=application/zip" "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart"
fi
